    ...
    private int purgeAscending(final int lowIndex, final int highIndex, final RollingFileManager manager) {
        ...
        for (int i = highIndex; i >= lowIndex; i--) {
            ...
            if (suffixLength > 0) {
                ...
                if (toRename.exists()) {
                    if (toRenameBase.exists()) {
                        LOGGER.debug("DefaultArchiveRolloverStrategy.purgeAscending deleting {} base of {}.", //
            ...
            if (toRename.exists()) {
                ...
                if (i == lowIndex) {
                    LOGGER.debug("DefaultArchiveRolloverStrategy.purgeAscending deleting {} at low index {}: all slots full.", //
                            toRename, i);
                    if (!toRename.delete()) {
                        return -1;
                    }
        ...
        for (int i = renames.size() - 1; i >= 0; i--) {
            ...
            try {
                LOGGER.debug("DefaultArchiveRolloverStrategy.purgeAscending executing {} of {}: {}", //
    ...
    private int purgeDescending(final int lowIndex, final int highIndex, final RollingFileManager manager) {
        ...
        for (int i = lowIndex; i <= highIndex; i++) {
            ...
            if (suffixLength > 0) {
                ...
                if (toRename.exists()) {
                    if (toRenameBase.exists()) {
                        LOGGER.debug("DefaultArchiveRolloverStrategy.purgeDescending deleting {} base of {}.", //
                                toRenameBase, toRename);
                        toRenameBase.delete();
                    }
                } else {
            ...
            if (toRename.exists()) {
                ...
                //        if that fails then abandon purge
                if (i == highIndex) {
                    LOGGER.debug("DefaultArchiveRolloverStrategy.purgeDescending deleting {} at high index {}: all slots full.", //
                            toRename, i);
                    if (!toRename.delete()) {
                        return -1;
                    }
        ...
        for (int i = renames.size() - 1; i >= 0; i--) {
            final Action action = renames.get(i);
            try {
                LOGGER.debug("DefaultArchiveRolloverStrategy.purgeDescending executing {} of {}: {}", //
                        i, renames.size(), action);
                if (!action.execute()) {
                    return -1;
                }
